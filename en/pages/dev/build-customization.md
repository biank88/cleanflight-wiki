...
The first file where you can specify the features that you want in your customized version, is the `target/common.h`.
...
```
If you're using, for example Serial RX, and a FlySky receiver (that uses iBus), you can comment (or remove) the lines for all the protocols not used, ending in something like:
```
#define USE_CLI
//#define USE_PPM
//#define USE_PWM
...
//#define USE_SERIALRX_XBUS       // JR
```
With this change, you have won space to add some missing features.
...
#endif
```
Again, you can comment or remove the things that you not use, and, if your flash is smaller than specified, you can move the lines of the features that you need out of the conditional. For example, you can end with something like this for a quadcopter with a NAZE32 (which has more than 64 but less than 128 memory flash size) and a FlySky receiver:
```
#if (FLASH_SIZE > 64)
#define BLACKBOX
#define LED_STRIP
...
#endif
```
As you can see, we've commented the telemetry protocols not used, the servos and take out of the conditional the GPS and iBus telemetry. Be careful, some features are dependent, for example, the `TELEMETRY_IBUS` needs the `TELEMETRY` enabled.
...
If you have some programming knowledge skills, you can modify this file. It's self-explanatory.
...
This file is located in `target/[FLIGHT_CONTROLLER_NAME]/target.h`.

In this file, there's a declaration of all the hardware supported by different versions of the flight controller, for example, in the `NAZE` file:
```
#define GYRO
...
```
Here you have different gyroscopes, accelerometers and barometers defined, to support all the variants of the flight controller. You can comment all the models that you're sure that your flight controller is not using.
